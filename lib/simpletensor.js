...
function NegativeOperation(node) {
    ...
    this.value = function (ctx) {
        return mapValues(node.value(ctx), function (value) { return -value; });
    }
}
...
function AddOperation(left, right) {
    ...
    this.value = function (ctx) {
        return mapValuesValues(left.value(ctx), right.value(ctx), function (x, y) { return x + y; });
    }
}
...
function PowOperation(left, right) {
    ...
    this.value = function (ctx) {
        return mapValuesValues(left.value(ctx), right.value(ctx), function (x, y) { return Math.pow(x, y); });
    }
}

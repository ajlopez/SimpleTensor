...
function AddOperation(left, right) {
	if (left.isNumber()) {
		if (right.isNumber()) {
			...
			this.value = function (ctx) { return left.value(ctx) + right.value(ctx); }
		}	
	}
		...
		if (right.isVector()) {
			...
			this.value = function (ctx) { 
				var value1 = left.value(ctx);
				var value2 = right.value(ctx);
				var l = value1.length;
	...
	this.evaluate = function (ctx) { return createConstantValue(this.value(ctx)); };
}
